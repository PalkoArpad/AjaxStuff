...
class Chat
{
    ...
    */
    ...
    {
        //escape the variable data for safely adding them to the db
    ...
    -$id parameter is sent by the client and it represents the id of the last
    message received by the client. Messages more recent by $id will be
    fetched from the database and returned to the client in JSON format
    */
    ...
    public function retrieveNewMessages($id=0)
    {
        ...
        if($result->num_rows){
            //loop through all the fetched messages to buld the result msg
    ...
    cleared since the last call to the server
    -$id parameter contains the id of the last message received by the client
    */
    private function isDatabaseCleared($id)
    {
        if($id > 0){
            //by checking the nr of rows with ids smaller than the
            ...
            //if a truncate operation occurred the whiteboard needs to reset
            if($row['old'] == 0)
                return 'true';
            return 'false';
        }
